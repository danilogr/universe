...
class mrp():
  ...
  def __init__(self,server):
    ...
    #get database
    self.database = re.search(b'\/web\/webclient\/js\?db=(\S*)"',content).group(1)

    #try to get a new session
    sessionreq='{"jsonrpc":"2.0","method":"call","params":{"session_id":null,"context":{}},"id":"%s"}'%self.__getid()
    req = urllib.request.Request('%s/web/session/get_session_info'%server)
    req.add_header('Content-Type', 'application/json')

    response = urllib.request.urlopen(req, sessionreq.encode('utf-8')).read().decode('utf-8')
  ...
  def __jsonrequest(self,path,params):
    ...
    req = urllib.request.Request('%s%s'%(self.server,path))
    req.add_header('Content-Type','application/json')

    response = urllib.request.urlopen(req,reqdict.encode('utf-8'))
  ...
  def login(self,username,password):
    ...
    if(self.__logged):
      ...
      params = {
          'db': self.database.decode(encoding='utf-8'),
          'login': username,
          'password': password,
